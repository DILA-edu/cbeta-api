...
class XMLForDocx2
  def initialize
    ...
    fn = Rails.root.join('log', 'xml4docx2.log')
    @log = File.open(fn, 'w')
  end
  ...
  def handle_p_contain_p(e, c)
    @log.puts "#{@fn} handle_p_contain_p"
    node = HTMLNode.new('p')
  ...
  def e_seg_seg(e, c)
    @log.puts "#{__LINE__} e_seg_seg, #{@fn}, #{c.to_xml}"
    node = HTMLNode.new('seg')
  ...
  def traverse(e)
    e.children.each do |c|
      @log.puts "#{@fn} #{c.content}" if c.comment?
      case c.name
