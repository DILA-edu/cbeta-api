...
namespace :convert do
  ...
  task :docx, %i[filter workers] => :environment do |_t, args|
    t1 = Time.now
    ConvertDocx.new(filter: args[:filter], workers: args[:workers]).convert
    puts ElapsedTime.label(t1)
  end
end

# 把 data/xml4docx 轉成 public/download/docx, 以多 process 平行處理
class ConvertDocx
  LOG_PATH = 'log/convert_docx.log'
  # 每種警告在 log 裡最多列幾個來源檔案
  WARNING_SAMPLE_SIZE = 5

  def initialize(filter: nil, workers: nil)
    @filter = filter
    @workers = (workers || Etc.nprocessors).to_i.clamp(1, 64)
    @src = Rails.root.join('data/xml4docx')
    @dest = Rails.root.join('public/download/docx')
  ...
  def convert
    ...
    prepare_dest
    puts "xml4docx 轉 docx: #{files.size} 檔, #{@workers} workers"
  ...
  def convert_files(files, total: nil)
    ...
    files.each_with_index do |src, i|
      ...
      dest.dirname.mkpath
      begin
        XmlToDocxConverter.new(src, figures_dir: @figures).convert(dest.to_s).each do |message|
  ...
  def print_progress(index, worker_total, total)
    ...
    print "\rconvert:docx #{done}/#{total}  "
  end

  def dest_path(src)
    relative = Pathname.new(src).relative_path_from(@src)
    @dest.join(relative.sub_ext('.docx'))
  end
